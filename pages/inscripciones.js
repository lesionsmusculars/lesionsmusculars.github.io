...
const Inscriptions = (props) => (
  ...
  <Layout>
    ...
    <section className='call-to-action'>
      ...
      {props.associationsitems.sort((a,b) => {
        if (a.id < b.id) {
            return -1
        }
        if (a.id > b.id) {
        ...
        return 0
        }).map((associationsitem, id) => (
                <li key={id}>
                  {associationsitem.name}
                </li>))}
        </ul>
...
Inscriptions.getInitialProps = async function () {
  const res = await fetch(`https://lesionsdata.now.sh/associations.json`)
  const associationsitems = await res.json()
  
  console.log(`Items de les associacions data fetched. Count: ${associationsitems.length}`)

  return { associationsitems }
